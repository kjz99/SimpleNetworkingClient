...
public class TcpSendConnection : TcpConnectionBase, IDisposable
{
    ...
    public TcpSendConnection(ISimpleNetworkingClientLogger logger, string host, int port, TimeSpan sendReadTimeout, int bufferSize, IList<byte> stxCharacters = default, IList<byte> etxCharacters = default) : base(logger, sendReadTimeout, bufferSize)
    {
        _host = host;
        _port = port;
